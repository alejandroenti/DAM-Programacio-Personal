...
pygame.display.set_caption('Alejandro López - Exercici 5')
...
# Iniciar el joc
def init_game():
    global balloons_list, balloons_dropped, balloons_caught, window_size, colors
    balloons_list = []
    balloons_dropped = 0
    balloons_caught = 0
...
def draw_balloon(balloon):
    ...
    pygame.draw.circle(screen, balloon["color"], center, balloon["radius"])

# Mostrar estadístiques
def display_stats():
    global font, screen, balloons_caught, balloons_dropped
    caught_string = font.render(f"Caught: {balloons_caught}", False, BLACK)
    dropped_string = font.render(f"Dropped: {balloons_dropped}", False, BLACK)

    screen.blit(caught_string, (10, 430))
    screen.blit(dropped_string, (10, 450))

# Actualitzar un globus individual
def update_balloon(balloon, delta_time):
    global window_size, balloons_dropped

    balloon["y"] += balloon["speed"] * delta_time

    if balloon["y"] - balloon["radius"] > window_size["height"]:
        balloons_dropped += 1
        init_balloon(balloon)
